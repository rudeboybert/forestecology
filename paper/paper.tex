...
understory, shrub layer) and drought tolerance (intolerant, resistant)
of the species.
...
\StringTok{  }\KeywordTok{select}\NormalTok{(}
    \DataTypeTok{sp =}\NormalTok{ spcode, family, genus, species, canopy_position, }
\NormalTok{    drought_tolerance}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{growth_scbi <-}\StringTok{ }\NormalTok{growth_scbi }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{left_join}\NormalTok{(sp_info, }\DataTypeTok{by =} \StringTok{"sp"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
\NormalTok{blocks_scbi <-}\StringTok{ }\KeywordTok{bind_rows}\NormalTok{(}
  \KeywordTok{sf_polygon}\NormalTok{(fold1), }\KeywordTok{sf_polygon}\NormalTok{(fold2), }
  \KeywordTok{sf_polygon}\NormalTok{(fold3), }\KeywordTok{sf_polygon}\NormalTok{(fold4)}
\NormalTok{) }\OperatorTok{%>%}
...
\NormalTok{spatial_block_scbi <-}\StringTok{ }
\StringTok{  }\KeywordTok{spatialBlock}\NormalTok{(}
    \DataTypeTok{speciesData =}\NormalTok{ growth_scbi, }\DataTypeTok{k =}\NormalTok{ n_fold, }
    \DataTypeTok{selection =} \StringTok{"systematic"}\NormalTok{, }\DataTypeTok{blocks =}\NormalTok{ blocks_scbi, }
...
\StringTok{  }\KeywordTok{create_focal_vs_comp}\NormalTok{(comp_dist, }\DataTypeTok{blocks =}\NormalTok{ blocks_scbi, }\DataTypeTok{id =} \StringTok{"stemID"}\NormalTok{, }\DataTypeTok{comp_x_var =} \StringTok{"basal_area"}\NormalTok{)}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{growth_hat =} \KeywordTok{predict}\NormalTok{(comp_bayes_lm_scbi, }
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{basal_area =} \FloatTok{0.0001} \OperatorTok{*}\StringTok{ }\NormalTok{pi }\OperatorTok{*}\StringTok{ }\NormalTok{(dbh1 }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{)}\OperatorTok{^}\DecValTok{2}\NormalTok{)}
...
\CommentTok{# Run all simulations}
\CommentTok{# 0. Setup simulation for this species type ----}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{growth_hat =} 
           \KeywordTok{predict}\NormalTok{(comp_bayes_lm_scbi, focal_vs_comp_scbi)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{rmse}\NormalTok{(}\DataTypeTok{truth =}\NormalTok{ growth, }\DataTypeTok{estimate =}\NormalTok{ growth_hat) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{pull}\NormalTok{(.estimate)}
...
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}
      \DataTypeTok{growth_hat =} \KeywordTok{predict}\NormalTok{(comp_bayes_lm_scbi, focal_vs_comp_scbi)}
\NormalTok{    ) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{rmse}\NormalTok{(}\DataTypeTok{truth =}\NormalTok{ growth, }\DataTypeTok{estimate =}\NormalTok{ growth_hat) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{pull}\NormalTok{(.estimate)}
\NormalTok{\}}
...
\StringTok{    }\KeywordTok{run_cv}\NormalTok{(}\DataTypeTok{comp_dist =}\NormalTok{ comp_dist, }\DataTypeTok{blocks =}\NormalTok{ blocks_scbi, }
           \DataTypeTok{run_shuffle =} \OtherTok{TRUE}\NormalTok{) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{rmse}\NormalTok{(}\DataTypeTok{truth =}\NormalTok{ growth, }\DataTypeTok{estimate =}\NormalTok{ growth_hat) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{pull}\NormalTok{(.estimate)}
  
  \CommentTok{# Status update}
  \KeywordTok{str_c}\NormalTok{(}\StringTok{"Shuffle with permutation "}\NormalTok{, j, }\StringTok{" at "}\NormalTok{, }\KeywordTok{Sys.time}\NormalTok{()) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{print}\NormalTok{()}
\NormalTok{\}}
...
\NormalTok{model_comp_tbl <-}\StringTok{ }\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{run_time =}\NormalTok{ run_time,}
  \DataTypeTok{observed_RMSE =}\NormalTok{ observed_RMSE,}
  \DataTypeTok{observed_RMSE_CV =}\NormalTok{ observed_RMSE_CV,}
  \DataTypeTok{shuffle_RMSE =}\NormalTok{ shuffle_RMSE,}
  \DataTypeTok{shuffle_RMSE_CV =}\NormalTok{ shuffle_RMSE_CV,}
\NormalTok{)}
\KeywordTok{save}\NormalTok{(model_comp_tbl, }\DataTypeTok{file =}\NormalTok{ filename }\OperatorTok{%>%}\StringTok{ }\KeywordTok{str_c}\NormalTok{(}\StringTok{".RData"}\NormalTok{))}


\CommentTok{# Visualize results ----------------------------------------------------}
\NormalTok{model_comp <-}\StringTok{ }\KeywordTok{bind_rows}\NormalTok{(}
\NormalTok{  model_comp_tbl }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{select}\NormalTok{(run_time, }\DataTypeTok{observed =}\NormalTok{ observed_RMSE, }
           \DataTypeTok{shuffle =}\NormalTok{ shuffle_RMSE) }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{CV =} \OtherTok{FALSE}\NormalTok{),}
\NormalTok{  model_comp_tbl }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{select}\NormalTok{(run_time, }\DataTypeTok{observed =}\NormalTok{ observed_RMSE_CV, }
           \DataTypeTok{shuffle =}\NormalTok{ shuffle_RMSE_CV) }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{CV =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{) }\OperatorTok{%>%}
...
\NormalTok{cv_plot <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_vline}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ model_comp_observed, }
             \KeywordTok{aes}\NormalTok{(}\DataTypeTok{xintercept =}\NormalTok{ RMSE, }\DataTypeTok{col =}\NormalTok{ CV), }
             \DataTypeTok{linetype =} \StringTok{"dashed"}\NormalTok{, }\DataTypeTok{show.legend =}\NormalTok{ F) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_histogram}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ model_comp_shuffle, }
                 \KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ RMSE, }\DataTypeTok{fill =}\NormalTok{ CV), }\DataTypeTok{bins =} \DecValTok{50}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}
    \DataTypeTok{fill =} \StringTok{"Cross-validated?"}\NormalTok{,}
    \DataTypeTok{x =} \KeywordTok{expression}\NormalTok{(}\KeywordTok{paste}\NormalTok{(}\StringTok{"RMSE (cm "}\NormalTok{, y}\OperatorTok{^}\NormalTok{\{}\OperatorTok{-}\DecValTok{1}\NormalTok{\}, }\StringTok{")"}\NormalTok{))}
\NormalTok{  ) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_viridis}\NormalTok{(}\DataTypeTok{discrete =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{option =} \StringTok{"D"}\NormalTok{) }\OperatorTok{+}
...
\StringTok{  }\KeywordTok{compute_growth}\NormalTok{(}
    ...
    \DataTypeTok{id =} \StringTok{"stemID"}
\NormalTok{  ) }\OperatorTok{%>%}
\StringTok{  }\CommentTok{# make all species the same, needs to be factor with at least two levels}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{sp =} \KeywordTok{factor}\NormalTok{(}\StringTok{'A'}\NormalTok{, }\DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}\StringTok{'A'}\NormalTok{, }\StringTok{'B'}\NormalTok{)) ) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\CommentTok{# Compute basal area:}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{basal_area =} \FloatTok{0.0001} \OperatorTok{*}\StringTok{ }\NormalTok{pi }\OperatorTok{*}\StringTok{ }\NormalTok{(dbh1 }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{)}\OperatorTok{^}\DecValTok{2}\NormalTok{)}

\NormalTok{study_region_scbi <-}\StringTok{ }\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{x =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{400}\NormalTok{, }\DecValTok{400}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{),}
  \DataTypeTok{y =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{400}\NormalTok{, }\DecValTok{400}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{sf_polygon}\NormalTok{()}
...
\CommentTok{# Associate each observation to a fold}
\NormalTok{spatial_block_scbi <-}\StringTok{ }
\StringTok{  }\KeywordTok{spatialBlock}\NormalTok{(}\DataTypeTok{speciesData =}\NormalTok{ growth_scbi, }\DataTypeTok{k =}\NormalTok{ n_fold, }
               \DataTypeTok{selection =} \StringTok{"systematic"}\NormalTok{, }\DataTypeTok{blocks =}\NormalTok{ blocks_scbi, }
               \DataTypeTok{showBlocks =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{verbose =} \OtherTok{FALSE}\NormalTok{)}

\NormalTok{growth_scbi <-}\StringTok{ }\NormalTok{growth_scbi }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{foldID =}\NormalTok{ spatial_block_scbi}\OperatorTok{$}\NormalTok{foldID }\OperatorTok{%>%}\StringTok{ }\KeywordTok{factor}\NormalTok{())}

\NormalTok{mult_dist_comp <-}\StringTok{ }\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{dist =} \KeywordTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{ ,}\FloatTok{6.25}\NormalTok{,  }\FloatTok{7.5}\NormalTok{, }\FloatTok{8.75}\NormalTok{,  }\DecValTok{10}\NormalTok{),}
  \DataTypeTok{rmse =} \DecValTok{0}
\NormalTok{)}
...
\NormalTok{  focal_vs_comp_scbi <-}\StringTok{ }\NormalTok{growth_scbi }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{create_focal_vs_comp}\NormalTok{(}\DataTypeTok{comp_dist =}\NormalTok{ comp_dist, }\DataTypeTok{blocks =}\NormalTok{ blocks_scbi, }\DataTypeTok{id =} \StringTok{"stemID"}\NormalTok{, }\DataTypeTok{comp_x_var =} \StringTok{"basal_area"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{run_cv}\NormalTok{(}\DataTypeTok{comp_dist =}\NormalTok{ comp_dist, }\DataTypeTok{blocks =}\NormalTok{ blocks_scbi)}
...
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(dist,rmse)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{xlab}\NormalTok{(}\StringTok{'Distance (m)'}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\StringTok{'RMSE (cm)'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{    }\KeywordTok{create_focal_vs_comp}\NormalTok{(}\DataTypeTok{comp_dist =}\NormalTok{ comp_dist, }
                         \DataTypeTok{blocks =}\NormalTok{ blocks_scbi, }
                         \DataTypeTok{id =} \StringTok{"stemID"}\NormalTok{, }
...
Creative use of the grouping variable allows for tests of what best
explains competitive interactions. Here we compare whether species
idenity or canopy position of the species best explains.
...
\StringTok{  }\KeywordTok{left_join}\NormalTok{(sp_info, }\DataTypeTok{by =} \StringTok{"sp"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    \DataTypeTok{canopy_position =} \KeywordTok{str_replace}\NormalTok{(canopy_position, }\StringTok{' '}\NormalTok{, }\StringTok{'_'}\NormalTok{),}
    \DataTypeTok{canopy_position =} \KeywordTok{str_replace}\NormalTok{(canopy_position, }\StringTok{','}\NormalTok{, }\StringTok{''}\NormalTok{),}
    \DataTypeTok{canopy_position =} \KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(canopy_position), }\StringTok{'shrub_layer'}\NormalTok{, canopy_position),}
    \DataTypeTok{sp =} \KeywordTok{as.factor}\NormalTok{(canopy_position),}
    \DataTypeTok{basal_area =} \FloatTok{0.0001} \OperatorTok{*}\StringTok{ }\NormalTok{pi }\OperatorTok{*}\StringTok{ }\NormalTok{(dbh1 }\OperatorTok{/}\StringTok{ }\DecValTok{2}\NormalTok{)}\OperatorTok{^}\DecValTok{2}\NormalTok{ )}
...
But we can still plot the competition posteriors for the canopoy
position groupings.
