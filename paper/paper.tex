...
Above we set the cut off distance for two stems to compete at 7.5 m. Our
package can be used to find which distance is best supported by the
data. Here we give an example.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mult_dist_comp <-}\StringTok{ }\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{dist =} \KeywordTok{c}\NormalTok{(}\FloatTok{2.5}\NormalTok{, }\DecValTok{5}\NormalTok{, }\FloatTok{7.5}\NormalTok{, }\DecValTok{10}\NormalTok{, }\FloatTok{12.5}\NormalTok{),}
