...
Above we set the cut off distance for two stems to compete at 7.5 m. Our package can be used to find which distance is best supported by the data. Here we give an example.

```{r appendix2-code}

mult_dist_comp <- tibble(
  dist = c(2.5, 5, 7.5, 10, 12.5 ),
  rmse = 0
)
...
for (i in 1:length(mult_dist_comp$dist))
{
  ...
  focal_vs_comp_scbi <- focal_vs_comp_scbi %>%
    run_cv(comp_dist = mult_dists[i], blocks = blocks_scbi)
