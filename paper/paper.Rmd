...
We hope to extend the `forestecology` package's functionality to account for a larger variety of models for tree growth and corresponding tests. Some immediate improvements that can made include
...
A recently emerging body of literature suggests that traits of species rather than species identity themselves better inform competition models [@kunstler_competitive_2012, @lasky_trait-mediated_2014]. To account for such clustering, instead of using Equation \ref{eq:model}, we present a model of which is a modification of @kunstler_plant_2016 in Equation \ref{eq:model2}. Let $Y_{ik}$ be the growth of the $k^{\text{th}}$ individual of species $i$ and let $j$ be the species of competitors in the neighborhood. How much growth changes per unit of $\text{BA}$ of all neighbors is quantified by two terms: an overall competition coefficient $C$ and an $i$-$j$ species pair specific coefficient $c_{ij}$. which depends on the traits of the $i$ and $j$ species. Say we have $l$ traits and $t_{li}$ is the value of trait $l$ for species $i$. $a_{\text{tol},l}$ quantifies how trait $l$ affects the way a species tolerates (or is sensitive to) competition. $a_{\text{eff},l}$ quantifies how trait $l$ is how much trait $l$ affects how strong a competitor is, how much it effects the reduction in growth of its neighbors. Finally $a_{\text{sim},l}$ is about whether competition is stronger (or weaker) if the two species are similar. This is based on an idea that two species that are similar use the same resources and should compete more.
