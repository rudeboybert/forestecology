...
- |
  ...
  | 3. We demonstrate the package's functionality using data from the Smithsonian Conservation Biology Institute's large forest dynamics plot, part of the ForestGEO global network of reseach sites. Given ForestGEO's data collection protocols and data formatting standards, the package was designed with cross-site compatibility in mind. We demonstrate that both i) competitor species identity matters and ii) that not spatially cross-validating leads to error estimates that are overly optimistic. 
...
keywords:
- forest ecology, interspecific competition, neigbhorhood competition, tree growth, R, ForestGEO, cross-validation, spatial statistics, 
...
The resulting RMSE of `r model_rmse_cv` computed using cross-validation is `r ifelse(model_rmse_cv > model_rmse, "larger", "smaller")` than the earlier RMSE of `r model_rmse`, suggesting that models that do not take the inherent spatial autocorrelation of the data into account generate error estimates that are overly optimistic; in our case RMSE's that are too low. We conduct an analysis of the full SCBI plot in Section \ref{results} below.  



# Results {#results}

Figure \ref{fig:scbi-simulation} highlights the importance of spatial cross validating when interpreting results from neighborhood compeition models. Without cross-validation the competitor idenity did matter, the non-permuted competitor species had a much lower RMSE than the permuted one. But once we include the spatial cross validation this improvement disaprees. So in this 9 ha subplot of the SCBI plot competitive interactions do not depend on the identity of the competitor, which is the opposite of what has been observed in other places (@allen_permutation_2020 @uriarte_spatially_2004).

```{r scbi-simulation, out.width="100%", fig.cap="Root mean squared error of models for standard, permuted, and spatial cross-validated error estimates.", echo = FALSE}
