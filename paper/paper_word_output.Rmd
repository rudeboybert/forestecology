...
focal_vs_comp_scbi <- focal_vs_comp_scbi %>%
  mutate(growth_hat = predict(comp_bayes_lm_scbi,
...
plot1 <- autoplot(comp_bayes_lm_scbi,
  type = "intercepts",
  sp_to_plot = sp_to_plot
)
plot2 <- autoplot(comp_bayes_lm_scbi,
  type = "dbh_slopes",
  sp_to_plot = sp_to_plot
)
plot3 <- autoplot(comp_bayes_lm_scbi,
  type = "competition",
  sp_to_plot = sp_to_plot
)
...
for (i in 1:length(mult_dist_comp$dist)) {
  ...
  focal_vs_comp_scbi <- growth_scbi %>%
    create_focal_vs_comp(comp_dist = comp_dist, blocks = blocks_scbi, id = "stemID", comp_x_var = "basal_area") %>%
    run_cv(comp_dist = comp_dist, blocks = blocks_scbi)
...
growth_scbi_can_pos <-
  ...
  mutate(
    ...
    canopy_position = ifelse(is.na(canopy_position), "shrub_layer", canopy_position),
...
```{r can-pos-comp-plot, out.width="100%", fig.cap="Posterior distributions of all competition parameters.", echo = FALSE}
