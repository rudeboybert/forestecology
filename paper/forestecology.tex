...
i) competitor species identity matters and ii) that not spatially
...
tree growth, R, ForestGEO, cross-validation, spatial statistics,
\vfill
...
based on neighborhood competition. It assumes a single large plot using
the ForestGEO plot data structure as a guide
...
Figure \ref{fig:scbi-simulation} highlights the importance of spatial
cross validating when interpreting results from neighborhood compeition
models. Without cross-validation the competitor idenity did matter, the
non-permuted competitor species had a much lower RMSE than the permuted
one. But once we include the spatial cross validation this improvement
disaprees. So in this 9 ha subplot of the SCBI plot competitive
interactions do not depend on the identity of the competitor, which is
the opposite of what has been observed in other places
(\citet{allen_permutation_2020} \citet{uriarte_spatially_2004}).
...
spatial structure., and S3 open-oriented model structure. These make for
ease of use. We hope that the package will further the use of
