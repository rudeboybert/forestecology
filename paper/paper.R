...
spatial_block_scbi <-
  cv_spatial(
    x = growth_scbi, k = n_fold,
    selection = "systematic", user_blocks = blocks_scbi,
    plot = FALSE, report = FALSE
  )

growth_scbi <- growth_scbi %>%
  mutate(foldID = spatial_block_scbi$folds_ids %>% factor())
...
#   user_blocks = blocks_scbi, plot = FALSE, report = FALSE
# )
# 
# growth_scbi <- growth_scbi %>%
